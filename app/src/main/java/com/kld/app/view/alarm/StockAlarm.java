 ...
 *@version 创建时间:2015-11-5下午09:48:07
 *类说明:库存预警
 */
public class StockAlarm {
	private static final Logger LOG = Logger.getLogger(StockAlarm.class);
	...
	public void setPanel(final JPanel centerPanel) throws Exception{
		panel=centerPanel;
		...
		JLabel hold1Label1 = new JLabel("预警类型：");
		...
		List<String> Namelst=dictService.selectYJLXByDictID(); //45表示预警类型
		ygbhcomboBox = new JComboBox();
		ygbhcomboBox.addItem("全部");//默认第一行为空
		for (String item:Namelst){
			ygbhcomboBox.addItem(item);
		}
		//ygbhcomboBox.addItem("未解警");
		...
		panel1.add(cxbutton);
		final AlarmInventoryService alarmInventoryService =
				(AlarmInventoryService) (Context.getInstance().getBean("alarmInventoryService"));
		Calendar calendar = new GregorianCalendar();
		calendar.setTime(endDate.getDate());
		calendar.add(calendar.DATE, 1);//
		list = alarmInventoryService.selectByDate(beginDate.getDate(),calendar.getTime(), "");
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					if (null == beginDate.getDate() || "".equals(beginDate.getDate())) {
						JOptionPane.showMessageDialog(null, "请输入开始时间！", "提示信息", JOptionPane.INFORMATION_MESSAGE);
						return;
					} else if (null == endDate.getDate() || "".equals(endDate.getDate())) {
						JOptionPane.showMessageDialog(null, "请输入结束时间！", "提示信息", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
					if (endDate.getDate().compareTo(beginDate.getDate()) < 0) {
						JOptionPane.showMessageDialog(null, "开始时间不能晚于结束时间,请重新选择", "提示信息", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
					//	SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm");
//					list = alarmInventoryService.selectByDate(beginDate.getDate(), endDate.getDate(),String.valueOf(ygbhcomboBox.getSelectedItem()));
					...
					list = alarmInventoryService.selectByDate(beginDate.getDate(), calendar.getTime(), Value);
					BindData(list);

				} catch (Exception ex) {
					LOG.error("查询库存预警记录失败", ex);
				}
			}
		});
	...
	private void BindData(List<HashMap<String,Object>> hashMaps)
	{
		data = new Object[hashMaps.size()][tableHeads.length];
		for (int i = 0; i < hashMaps.size(); i++) {
			HashMap<String,Object> info = hashMaps.get(i);
			data[i][0] = info.get("OilCan");
			data[i][1] = info.get("Name");
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String starttime="";
			String endtime="";
			if(info.get("StartTime")!=null&&!StringUtils.isEmpty(info.get("StartTime").toString()))
			{
				starttime=sdf.format(info.get("StartTime"));
			}
			if(info.get("EndTIme")!=null&&!StringUtils.isEmpty(info.get("EndTIme").toString()))
			{
				endtime=sdf.format(info.get("EndTIme"));
			}
			data[i][2] = starttime;
			data[i][3] = endtime;
		}
